...
module.exports = function (file) {
  return new Promise((resolve, reject) => {
    file = [file];
    if (opts.args?.opw == false && opts.args.upw) {
      opts.args.opw = opts.args.upw;
    }

    if (opts.args?.upw == false && opts.args.opw) {
      opts.args.upw = opts.args.opw;
    }

    Object.entries(opts.args).filter((el) => {
      file.push(`-${el[0]}`, el[1]);
    });

    execFile(
      path.join(popplerPath, "pdfimages"),
      [...file, ...["-list"]],
      EXEC_OPTS,
      (error, stdout, stderr) => {
        if (error) {
